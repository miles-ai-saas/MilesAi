...
- ``builtin.py``：按 slug 分发至 ``tenant.tools.builtins.handlers``。
- ``custom.py``：租户 HTTP / 脚本工具。

不在此包内
----------
``tenant.tools.builtins/``：各内置工具的 handler 实现（``calculator``、``generate_image`` 等）。

无 ``XxxService`` 类；为纯函数 + 门面 re-export。对外::

    from app.tenant.tools.invoke import invoke_tool_with_context, safe_calculate
"""
...
from app.tenant.tools.invoke.context import (
    invoke_tool_by_name,
    invoke_tool_with_context,
    resolve_bound_skill_id_from_agent,
)
from app.tenant.tools.invoke.custom import (
