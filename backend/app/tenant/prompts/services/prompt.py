...
class PromptService(BaseService):
    ...
    async def list_templates(
        ...
        *,
        ...
        tag_subq = TagService(self.db, self.ctx).entity_id_filter(
            TagEntityType.PROMPT, tag_ids or []
        )
        ...
        cat_names = await CategoryService(self.db, self.ctx).get_category_name_map(
            CategoryDomain.PROMPT, cat_ids
        )
        tags_map = await TagService(self.db, self.ctx).get_refs_map(
            TagEntityType.PROMPT, {i.id for i in items}
        )
        return PageResult(
    ...
    async def create_template(self, body: PromptTemplateCreate) -> PromptTemplateOut:
        await CategoryService(self.db, self.ctx).validate_category_for_domain(
            body.category_id, CategoryDomain.PROMPT
        )
        row = PromptTemplate(
        ...
        if body.tag_ids:
            await TagService(self.db, self.ctx).replace_entity_tags(
                TagEntityType.PROMPT, row.id, body.tag_ids
            )
        await self.db.refresh(row)
        tags_map = await TagService(self.db, self.ctx).get_refs_map(
            TagEntityType.PROMPT, {row.id}
        )
    ...
    async def update_template(self, template_id: UUID, body: PromptTemplateUpdate) -> PromptTemplateOut:
        ...
        if "category_id" in data:
            await CategoryService(self.db, self.ctx).validate_category_for_domain(
                data.get("category_id"), CategoryDomain.PROMPT
            )
        ...
        if tag_ids is not None:
            await TagService(self.db, self.ctx).replace_entity_tags(
                TagEntityType.PROMPT, row.id, tag_ids
            )
        await self.db.refresh(row)
        cat_names = await CategoryService(self.db, self.ctx).get_category_name_map(
