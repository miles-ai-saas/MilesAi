...
def _seed_choices() -> list[str]:
    from miles_server.scripts.db_ops import SEED_TARGETS
...
def migrate() -> None:
    ...
    from miles_server.scripts.db_ops import run_migrate
...
def init_db(migrate_only: bool, seed_only: bool) -> None:
    ...
    from miles_server.scripts.db_ops import run_init_db_sync
...
def seed(target: str) -> None:
    ...
    from miles_server.scripts.db_ops import run_seed_sync
...
def verify_db() -> None:
    """检查迁移版本与核心表是否齐全。"""
    from miles_server.scripts.verify_db import main as verify_main
...
def backfill_media_assets(
    ...
    from miles_server.scripts.backfill_media_assets import run_backfill_media_assets
