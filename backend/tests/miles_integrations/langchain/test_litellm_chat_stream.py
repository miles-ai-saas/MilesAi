...
from miles_integrations.litellm.adapter import litellm_chat_completion_stream
...
async def test_litellm_chat_completion_stream_calls_on_delta_and_returns_full():
    ...
    with (
        patch("miles_integrations.litellm.adapter._import_litellm") as imp,
        patch("miles_integrations.litellm.adapter.resolve_litellm_model", return_value="openai/gpt-test"),
        patch("miles_integrations.litellm.adapter._ensure_messages_valid_for_chat"),
        patch("miles_integrations.litellm.adapter._resolve_api_base", return_value=None),
    ):
        litellm = MagicMock()
...
async def test_ainvoke_chat_uses_stream_when_on_delta_set():
    ...
    with (
        patch(
            "miles_integrations.langchain.chat_models.litellm_chat_completion_stream",
            new_callable=AsyncMock,
            return_value="streamed",
        ) as mock_stream,
        patch(
            "miles_integrations.langchain.chat_models.litellm_chat_completion",
            new_callable=AsyncMock,
        ) as mock_non_stream,
    ):
