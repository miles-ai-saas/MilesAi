...
from miles_integrations.generative.video.providers.volcengine_video import (
    _build_request_body,
    generate_volcengine_video,
)
from miles_integrations.generative.volcengine_client import (
...
def test_build_content_last_without_first_raises():
    with pytest.raises(BadRequestError, match="首尾帧"):
        from miles_integrations.generative.video.providers.volcengine_video import _build_content
...
async def test_generate_volcengine_video_happy_path():
    ...
    with (
        ...
        patch(
            "miles_integrations.generative.video.providers.volcengine_video.poll_volcengine_video_task",
            new_callable=AsyncMock,
            return_value={"status": "succeeded", "content": {"video_url": "https://example.com/v.mp4"}},
        ),
        patch(
            "miles_integrations.generative.video.providers.volcengine_video.download_remote_bytes",
            new_callable=AsyncMock,
            return_value=mp4,
        ),
    ):
...
async def test_generate_video_for_model_doubao_route():
    ...
    with (
        ...
        patch(
            "miles_integrations.generative.video.service.generate_volcengine_video",
            new_callable=AsyncMock,
