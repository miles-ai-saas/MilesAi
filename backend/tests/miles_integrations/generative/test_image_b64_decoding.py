...
from miles_integrations.generative.image.providers.openai_images import generate_openai_images
from miles_integrations.generative.image.providers.volcengine_image import generate_volcengine_image
...
async def test_volcengine_all_dirty_reports_readable_error():
    ...
    with patch(
        "miles_integrations.generative.image.providers.volcengine_image.httpx.AsyncClient",
        return_value=client,
    ):
...
async def test_openai_images_all_dirty_reports_readable_error():
    client = _mock_client({"data": [{"b64_json": _GARBAGE}]})
    with patch(
        "miles_integrations.generative.image.providers.openai_images.httpx.AsyncClient",
        return_value=client,
    ):
        with pytest.raises(AppError, match="未返回可用图片数据"):
