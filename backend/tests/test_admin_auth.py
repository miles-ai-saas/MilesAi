...
async def test_validate_admin_session_ok():
    admin_id = uuid4()
    jti = "test-jti"
    with patch("app.admin.app_sys.session_store.get_redis") as mock_redis_fn:
        redis = AsyncMock()
...
async def test_change_password_revokes_session():
    ...
    with (
        patch("app.admin.app_sys.services.auth.verify_password", return_value=True),
        patch("app.admin.app_sys.services.auth.hash_password", return_value="newhash"),
        patch("app.admin.app_sys.services.auth.revoke_admin_session", new_callable=AsyncMock) as revoke,
    ):
