...
export function Pagination({ page, size, total, onPageChange, onSizeChange, pageSizeOptions = PAGE_SIZE_OPTIONS, className = "" }: PaginationProps) {
  ...
  const options = pageSizeOptions.includes(safeSize) ? pageSizeOptions : [...pageSizeOptions, safeSize].sort((a, b) => a - b);

  return (
    <nav aria-label="列表分页" className={`flex flex-wrap items-center justify-end gap-x-3 gap-y-2 text-xs text-ink-muted ${className}`}>
      ...
      <div className="inline-flex items-stretch overflow-hidden rounded-md border border-line bg-surface shadow-sm">
        <button type="button" disabled={safePage <= 1} onClick={() => onPageChange(safePage - 1)} className={navBtnClass} aria-label="上一页">
          ‹
        </button>
        <span className="flex min-w-[3.25rem] items-center justify-center border-x border-line px-2 py-1.5 tabular-nums text-ink" aria-live="polite">
          {safePage} / {pages}
        </span>
        <button type="button" disabled={safePage >= pages} onClick={() => onPageChange(safePage + 1)} className={navBtnClass} aria-label="下一页">
          ›
        </button>
      </div>
    </nav>
