...
export function UserMenu({ variant = "sidebar" }: { variant?: "sidebar" | "header" }) {
  ...
  return (
    <div ref={rootRef} className={`relative ${isHeader ? "" : "border-t border-line-soft p-3"}`}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        {isHeader && <span className="hidden max-w-[120px] truncate text-sm font-medium text-ink md:inline">{user.username}</span>}
      ...
      {open && (
        <div
          ...
          className={`absolute z-50 overflow-hidden rounded-lg border border-line bg-surface py-1 shadow-panel ${
            isHeader ? "right-0 top-full mt-1 w-48" : "bottom-full left-3 right-3 mb-1"
          }`}
        >
          {isHeader && (
          ...
          {user.is_superuser && <p className="border-b border-line-soft px-3 py-2 text-xs text-brand">超级管理员</p>}
          <button
            type="button"
            role="menuitem"
