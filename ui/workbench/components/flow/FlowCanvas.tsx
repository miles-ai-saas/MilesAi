...
import { PALETTE_GROUPS, paletteItemsByGroup, type NodeType, createPaletteNode, graphToReactFlow, reactFlowToGraph } from "@/lib/flow-nodes";
...
const FlowCanvasInner = forwardRef<FlowCanvasHandle, FlowCanvasProps>(function FlowCanvasInner(
  ...
  const insertTemplate = useCallback((tpl: FlowTemplate) => confirmReplaceGraph(`插入「${tpl.label}」模板`, tpl.graph_json), [confirmReplaceGraph]);
        ...
        {paletteOpen ? (
          <aside className="flex w-44 shrink-0 flex-col border-r border-line bg-surface sm:w-48">
            <div className="flex items-center justify-between border-b border-line px-2 py-2">
              ...
              <button type="button" className="btn-sm-ghost !px-1.5 text-[10px]" onClick={() => setPaletteOpen(false)} title="收起节点面板">
                ‹
              </button>
            </div>
