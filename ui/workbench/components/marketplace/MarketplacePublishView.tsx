        ...
        <section className="rounded-xl border border-line bg-surface p-6 shadow-panel">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <label className="block space-y-1">
              ...
              <input
                className="input-field w-full"
                value={vm.publishIcon}
                onChange={(e) => vm.setPublishIcon(e.target.value)}
              />
            </label>
            <label className="block space-y-1">
              ...
              <select
                className="input-field w-full"
                value={vm.publishCategory}
                onChange={(e) => vm.setPublishCategory(e.target.value)}
              >
                {vm.categories.map((c) => (
          ...
          <label className="mt-3 block space-y-1">
            ...
            >
              ...
              <option value="tenant_only">
                {marketplaceVisibilityLabel("tenant_only", vm.marketplaceMeta)}
              </option>
            </select>
          ...
          <div className="mt-2 space-y-2">
            <select
              className="input-field w-full"
              value={vm.publishKbId}
              onChange={(e) => vm.setPublishKbId(e.target.value)}
            >
            ...
            <select
              className="input-field w-full"
              value={vm.publishFlowId}
              onChange={(e) => vm.setPublishFlowId(e.target.value)}
            >
            ...
            <select
              className="input-field w-full"
              value={vm.publishAgentId}
              onChange={(e) => vm.setPublishAgentId(e.target.value)}
            >
          ...
          <button
            type="button"
            disabled={vm.publishLoading}
            onClick={vm.onCreateDraft}
