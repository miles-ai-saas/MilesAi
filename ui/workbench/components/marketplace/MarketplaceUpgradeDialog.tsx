...
function DiffValue({ value, changed }: { value: string | null | undefined; changed: boolean }) {
  ...
  return <span className={changed ? "text-amber-700 dark:text-amber-300" : "text-ink-muted"}>{value}</span>;
}

export function MarketplaceUpgradeDialog({ open, loading, preview, upgrading, onClose, onConfirm, mode = "upgrade" }: Props) {
  const isRollback = mode === "rollback";
  const title = preview ? (isRollback ? `回滚 ${preview.app_name}` : `升级 ${preview.app_name}`) : isRollback ? "应用回滚预览" : "应用升级预览";
  const description = preview ? `v${preview.installed_version} → v${preview.target_version}` : undefined;
      ...
      footer={
        <>
          ...
          <button type="button" className="btn-primary" onClick={onConfirm} disabled={!canConfirm}>
            {upgrading ? (isRollback ? "回滚中…" : "升级中…") : isRollback ? "确认回滚" : "确认升级"}
          </button>
        </>
      }
    >
      {loading ? (
        ...
        <div className="space-y-4">
          {preview.message ? <p className="rounded-lg border border-line bg-surface-muted/40 px-4 py-3 text-sm text-ink-muted">{preview.message}</p> : null}
                ...
                <header className="flex flex-wrap items-center justify-between gap-2 border-b border-line px-4 py-3">
                  ...
                  <span
                    className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      resource.has_changes ? "bg-amber-100 text-amber-800 dark:bg-amber-950/50 dark:text-amber-200" : "bg-surface-muted text-ink-muted"
                    }`}
                  >
                  ...
                  <table className="w-full min-w-[480px] text-sm">
                    ...
                    <tbody>
                      {resource.changes.map((change) => (
                        <tr key={change.field} className={change.changed ? "bg-amber-50/50 dark:bg-amber-950/20" : "border-b border-line/60"}>
