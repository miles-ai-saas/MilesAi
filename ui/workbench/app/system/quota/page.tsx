...
function QuotaCard({
  ...
  return (
    <div className="card p-4">
      ...
      {unlimited && (
        <p className="mt-2 text-xs text-ink-faint">当前未设置上限</p>
      )}
    </div>
  );
}
...
export default function SystemQuotaPage() {
  ...
  return (
    <div className="w-full space-y-6">
      <PageHeader
        title="资源配额"
        description="查看本租户资源使用与上限。配额调整请联系平台管理员（运营后台）。"
      />
      ...
      <p className="text-xs text-ink-faint">
        如需提升配额，请联系平台管理员在运营后台调整租户套餐或配额上限。
      </p>
    </div>
  );
}
