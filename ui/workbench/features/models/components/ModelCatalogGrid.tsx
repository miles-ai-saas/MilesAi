      ...
      {vm.items.map((m) => (
        <article key={m.id} className="resource-card relative !min-h-0 flex-col !items-stretch !p-4">
          ...
          <div className="mb-2 flex items-center gap-2">
            <span className="rounded bg-surface-muted px-2 py-0.5 text-xs font-medium text-ink">{modelVendorLabel(m.vendor, vm.meta)}</span>
            <span className="text-xs text-ink-muted">{m.model_code ?? m.model_name}</span>
          </div>
          <h3 className="pr-12 text-base font-semibold text-ink">
            <button type="button" className="text-left hover:text-brand" onClick={() => vm.openDetail(m.id)}>
              {m.name}
            </button>
          </h3>
          ...
          <p className="mt-2 text-xs text-ink-muted">{modelCredentialHint(m)}</p>
          <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-3">
            <button type="button" className="btn-ghost text-xs" onClick={() => vm.openDetail(m.id)}>
              查看详情
            </button>
            {m.source === "builtin" ? (
              isBuiltinByok(m) ? (
