...
export default function AuditPage() {
  ...
  const dateRange = useMemo(
    () => auditDateRangeFromPreset(datePreset, customFrom, customTo),
      ...
      <div className="mb-4 flex flex-wrap items-end gap-2">
        ...
        <label className="flex flex-col gap-1">
          ...
          <select
            className="input-field w-auto min-w-[8rem]"
            value={datePreset}
            onChange={(e) => setDatePreset(e.target.value as AuditDatePreset)}
          >
            {DATE_PRESETS.map((p) => (
              ...
              {list.items.map((l) => (
                <li key={l.id} className="admin-data-row">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {l.admin_username && (
                      <span className="badge bg-brand-light text-ink">{l.admin_username}</span>
                    )}
                    {l.tenant_id && (
