"use client";

import Link from "next/link";
import { ListFooter } from "@/components/list/ListFooter";
import type { TenantsPageVm } from "@/features/tenant/hooks/use-tenants-page";
import { TENANT_STATUS_FILTERS } from "@/features/tenant/lib/tenants-page-shared";
import { TENANT_STATUS_LABEL } from "@/features/tenant/lib/tenant-detail-shared";

export function TenantsStatusFilterSection({ vm }: { vm: TenantsPageVm }) {
  const { statusFilter, setStatusFilter } = vm;

  return (
    <div className="mb-4 flex flex-wrap gap-2">
      {TENANT_STATUS_FILTERS.map((s) => (
        <button
          key={s || "all"}
          type="button"
          onClick={() => setStatusFilter(s)}
          className={`rounded-full px-3 py-1 text-xs ${statusFilter === s ? "bg-brand text-white" : "border bg-white text-ink-muted"}`}
        >
          {s ? TENANT_STATUS_LABEL[s] || s : "全部"}
        </button>
      ))}
    </div>
  );
}

export function TenantsCreateSection({ vm }: { vm: TenantsPageVm }) {
  const { plans, name, setName, planId, setPlanId, create } = vm;

  return (
    <section className="card mb-6 p-4">
      <h2 className="text-sm font-semibold">新建租户</h2>
      <div className="mt-3 flex flex-wrap gap-2">
        <input className="input-field max-w-xs" placeholder="租户名称" value={name} onChange={(e) => setName(e.target.value)} />
        <select className="input-field max-w-xs" value={planId} onChange={(e) => setPlanId(e.target.value)}>
          <option value="">选择套餐</option>
          {plans.map((p) => (
            <option key={p.id} value={p.id}>
              {p.name}
            </option>
          ))}
        </select>
        <button type="button" onClick={() => void create()} className="btn-primary">
          创建
        </button>
      </div>
    </section>
  );
}

export function TenantsTableSection({ vm }: { vm: TenantsPageVm }) {
  const { list } = vm;

  if (list.loading) {
    return <p className="text-sm text-ink-muted">加载中…</p>;
  }

  return (
    <>
      <div className="admin-table-wrap">
        <table className="admin-table">
          <thead>
            <tr>
              <th>名称</th>
              <th className="col-center">套餐</th>
              <th className="col-center">状态</th>
              <th className="col-center col-numeric">用量</th>
              <th className="col-actions">操作</th>
            </tr>
          </thead>
          <tbody>
            {list.items.length === 0 && (
              <tr>
                <td colSpan={5} className="py-10 text-center cell-muted">
                  暂无租户
                </td>
              </tr>
            )}
            {list.items.map((t) => (
              <tr key={t.id}>
                <td className="cell-primary">{t.name}</td>
                <td className="col-center cell-muted">{t.plan_name || "—"}</td>
                <td className="col-center">
                  <span className="badge bg-brand-light text-ink">{TENANT_STATUS_LABEL[t.status] || t.status}</span>
                </td>
                <td className="col-center col-numeric cell-numeric">
                  {t.storage_used_mb}/{t.max_storage_mb} MB · Token {t.tokens_used_month.toLocaleString()}
                </td>
                <td className="col-actions">
                  <Link href={`/tenants/detail?id=${t.id}`} className="text-brand hover:underline">
                    详情
                  </Link>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <ListFooter className="mt-3" page={list.page} size={list.size} total={list.total} onPageChange={list.setPage} onSizeChange={list.setSize} />
    </>
  );
}
