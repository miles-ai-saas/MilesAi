  ...
  return (
    <div
      className={`rounded-lg border p-4 ${
        over ? "border-amber-300 bg-amber-50/40" : "border-line-soft bg-surface"
      }`}
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
            ...
            <span
              className={`cell-numeric text-xs ${warn || over ? "font-medium text-amber-700" : "cell-muted"}`}
            >
              {used.toLocaleString()}
          ...
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-line-soft">
            <div
              className={`h-full rounded-full transition-all ${
                over ? "bg-red-500" : warn ? "bg-amber-500" : "bg-brand"
              }`}
        ...
        <label className="shrink-0 sm:w-32">
          ...
          <input
            type="number"
            min={0}
            className="input-field mt-1 w-full sm:w-32"
            value={max}
