...
import { EMPTY_RATE_LIMIT_RULE_FORM, type RateLimitRuleForm } from "@/lib/risk-page-shared";
...
export function useRiskPage() {
  ...
  const events = usePagedList(useCallback((p, s) => adminApi.listRiskEvents(p, s), []), { enabled: ready });
  const ips = usePagedList(useCallback((p, s) => adminApi.listIpBlacklist(p, s), []), { enabled: ready });
  const rules = usePagedList(useCallback((p, s) => adminApi.listRateLimits(p, s), []), { enabled: ready });
