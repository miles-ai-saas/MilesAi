...
- 2026-09-16 实施：32 处迁移完成，UP042 启用（`ruff check --select UP042 .` 收敛为 0）。
  ...
  真库探针写入/读回一致，敏感度对照（`--corrupt`）按期报错。§5.2 的结论依据是**结构性**的
  （只换基类；成员值 / `values_callable` / Alembic 文件零改动 ⇒ DDL 与落库字符串对全部 32 处
  构造性不变），§7.1/§7.3 的 2 + 3 个代表列只是**抽样复核**，不是归纳依据。
```

> **回写勘误（Task 2 评审后）**：本模板初稿写的是「spec §5.2 的条件已满足：迁移未产生任何 DB 层差异」。
> 该措辞让结论显得由抽样归纳得出（真库往返仅 2 列、SAEnum 断言仅 3 列，而唯一覆盖全 schema 的
> `alembic check` 又未取得结论），属「claim 宽于 cited evidence」。已改写为上面这段——把依据落在
> 结构性论证上，并显式标注抽样性质。**若 `alembic check` 未取得结论，另起一条如实记录**：须区分
> 「环境性滞后」（如本地库落后于 head）与「真 DDL 漂移」，不得并入上一条的"已满足"里。
