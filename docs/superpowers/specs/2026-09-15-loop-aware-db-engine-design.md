...
- ~~`get_redis()` 的 `id(loop)` 复用隐患（本设计不复制该做法，但不在本专项修）。~~ → **已修**（2026-09-18，commit `c6c02085`）：`infra/redis/client.py` 的守卫改为持有 loop 对象并以 `is` 比较，与 §5 同判据；见 §10。本设计当初「不并入 `run_worker_db_coro`」的解耦结论不变，只换了守卫的判据。

> 该条与本设计 §2 的判据同源：`id()` 复用会让「同一个 loop」判定误成立。补一句实测边界，
> 以免后人高估其紧迫性：旧实现下旧 loop **确实**会被回收（weakref 确认——Redis 客户端不像
> asyncpg 那样强引用 loop），地址复用结构上可能，但按生产形状模拟（客户端缓存 + loop 轮换）
> 实测 0/600~0/3000，释放后批量分配 3000 个 loop 亦 0 次撞回，**构造不出**守卫误判。故该修
> 属消除「偶然安全」，而非修复可复现的活缺陷；这也解释了它为何能长期作为遗留项存在。
...
- 2026-09-18 **闭合 §9 第 1 条**（commit `c6c02085`）：`get_redis()` 的 loop 守卫由
  `_redis_loop_id != id(running_loop)` 改为持有 loop 对象并以 `is` 比较，与本设计 §5 的判据
  一致（`async_session.py` 早已写明该理由，Redis 侧一直沿用 id 比较）。缓存的生存期语义不变
  （仍由 `reset_redis()` 释放、仍是单例），调用方无感知。判定表逐项对照通过（冷启动 / 同 loop
  复用 / 换 loop 重建 / 同步上下文无 loop 时复用 / `reset_redis` 后重建）。
  **如实记录其紧迫性**：这是消除「偶然安全」而非修复可复现的活缺陷——旧实现下旧 loop 确实会被
  回收（weakref 确认，Redis 客户端不像 asyncpg 那样强引用 loop），地址复用结构上可能（纯 loop
  场景实测 1.15%；每轮额外分配一个大对象时 99.95%），但按生产形状模拟（客户端缓存 + loop 轮换）
  实测 0/600~0/3000、释放后批量分配 3000 个 loop 亦 0 次撞回，未能构造出守卫误判。唯一可复现的
  失效形态是两个并存活 loop 交替调用（12 次调用重建 11 次客户端且旧客户端未 `aclose()`），而
  `packages/` 内无任何 `new_event_loop`，生产不存在该形态。新守卫因强引用钉住旧 loop，使地址
  在解析上也不可能复用，故该失效形状在两个方向上都被消除。
