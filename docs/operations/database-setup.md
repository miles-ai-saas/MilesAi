...
3. 后端依赖已安装：`cd backend && uv sync --all-packages --group dev`。
...
> **说明：** 启动 API 时（`uvicorn miles_server.main:app`）也会在 lifespan 中自动执行 `alembic upgrade head`。手动执行一遍更便于排错；生产环境建议部署流程中 **显式跑迁移**，不 sole 依赖启动时迁移。

**从旧多文件迁移（002–021）升级到此版本：**

- 若表结构已与当前代码一致：`cd backend && alembic stamp 001`
- 若需干净库：删库重建后 `alembic upgrade head`，再 `milesai init-db --seed-only`
...
```bash
cd backend
milesai init-db              # 迁移 + 全量种子（若已迁移过可 --seed-only）
milesai init-db --seed-only
milesai seed all             # 仅种子，等价于 init-db --seed-only
```
...
种子实现位于 `backend/packages/miles-server/src/miles_server/scripts/seed/`（租户、合规、应用市场、运营账号、内置模型目录等）。可按域单独执行：`milesai seed tenant` 等。

### 工作台分类（`sys_categories`）

默认定义见 [`backend/packages/miles-server/src/miles_server/scripts/seed/data/sys_categories_defaults.json`](../../backend/packages/miles-server/src/miles_server/scripts/seed/data/sys_categories_defaults.json)：
...
```bash
cd backend
milesai seed categories   # 为所有租户补齐三域分类
milesai seed tenant       # 新建租户时也会调用 seed_categories_for_tenant
```

`seed all` / `init-db` 已包含 `seed_categories`。

```bash
uvicorn miles_server.main:app --reload --port 8000
```
...
应用内自动拼接（见 `backend/packages/miles-core/src/miles_core/config.py`）：
...
| `upgrade head` 无 `Running upgrade` 且没有表 | 运行 `milesai verify-db`；缺表则按「重建库」清 schema 后 `milesai init-db` |
