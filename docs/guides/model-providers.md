...
`ModelConfig` → `resolve_model_for_invoke`（租户 BYOK）→ `integrations.litellm.adapter.litellm_chat_completion`（`litellm.acompletion`）→ `integrations.langchain.chat_models.ainvoke_chat`。
