...
| 范围 | 路径 | 内容 |
|------|------|------|
| 跨类型共用键 | `app/common/constants/model_extra.py` | `EXTRA_INVOKE_MODE`（值 `"invoke_mode"`） |
| 向量化 | `app/integrations/embeddings/constants.py` | `INVOKE_MODE_*`、`EXTRA_EMBEDDING_*` |
| 重排序 | `app/integrations/rerank/constants.py` | `INVOKE_MODE_*`、`EXTRA_RERANK_*` |
