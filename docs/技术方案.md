...
10. [Langflow 内嵌方案](#10-langflow-内嵌方案)
11. [安全合规实现](#11-安全合规实现)
12. [前端技术方案](#12-前端技术方案)
13. [部署架构](#13-部署架构)
14. [非功能需求](#14-非功能需求)
...
| Langflow 同进程 | 内嵌 Langflow Runtime，无 iframe、无独立服务端口 |
...
| 模块4 智能体编排 | `services/agents/`, `services/flows/` | 智能体、Langflow 流程版本 |
...
| Langflow | `FlowRuntime.run/save/load` | API、智能体执行 |
...
| 流程 | `GET/POST /flows`, `POST /flows/{id}/run`, `POST /flows/{id}/publish` | Langflow |
...
1. Langflow 作为 Python 依赖安装，在应用启动时初始化 `LangflowApplication`（具体 API 以锁定版本为准）。
2. FastAPI 挂载内部路由 `/internal/langflow/*`（Service Token 或内网限定）。
3. 流程图 JSON 持久化至 `flows` + `flow_versions`，与 Langflow 内部 ID 映射。
4. 前端 `@langflow/flow-builder` 经 BFF（Next.js API Route 或直连后端）读写流程，**无 iframe**。

### 10.2 自定义节点（`custom_nodes/`）
...
### 10.4 Spike 验证项（P2 前必做）

- [ ] 同进程启动无端口冲突
- [ ] `@langflow/flow-builder` 与后端 flow API 联调
- [ ] 自定义节点注册与调试执行
...
- `FlowEditor`：封装 flow-builder，对接 `/flows` API
...
services:
  web:           # Next.js :3000
  api:           # FastAPI :8000（含 Langflow 内嵌）
  worker-parse:
...
| **P2 编排与智能体** | 3–4 周 | Langflow 内嵌、自定义节点、智能体 | 画布无 iframe；流程可发布运行 |
...
| 风险 | 影响 | 对策 |
|------|------|------|
| Langflow 版本 API 变动 | 编排不可用 | 锁定版本；`FlowRuntime` 适配层抽象 |
...
本方案技术选型与需求 **§2 完全一致，不可修改**：

**后端：** FastAPI · Langflow（内嵌）· Docling · PaddleOCR · Whisper · Weaviate · PostgreSQL 15+ · MinIO · Redis 7+ · Celery · JWT+RBAC · SQLAlchemy · Docker Compose

**前端：** Next.js 14 · TypeScript · TailwindCSS · `@langflow/flow-builder` · Axios
...
- [x] [数据库初始化.md](./数据库初始化.md) — PostgreSQL 建库、Alembic 迁移、种子数据
