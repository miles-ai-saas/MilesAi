...
**多子智能体（规划）**：关联表 `agt_sub_agent_bindings`；绑定后强制 `runtime_mode=autonomous`，禁止用 legacy/workflow 做主调度。详见 [agent-enhancement-langchain-stack.md](./agent-enhancement-langchain-stack.md) §3.2。

实施节奏建议 **P6**（Spike → LangChain 底座 → LangGraph → 画布编译 → DeepAgents + 子智能体绑定），全程保留 `runtime_mode=legacy` 回滚（无子智能体绑定时）。
