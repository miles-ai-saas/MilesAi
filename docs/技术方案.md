...
> **实现说明（编排）**：`app/flow_runtime/` 节点 + LangGraph 编译执行；前端 React Flow 画布，详见 [flow-runtime.md](./flow-runtime.md)。
...
10. [流程运行时（LangGraph）](#10-流程运行时langgraph)
11. [安全合规实现](#11-安全合规实现)
12. [前端技术方案](#12-前端技术方案)
13. [部署架构](#13-部署架构)
14. [非功能需求](#14-非功能需求)
...
| 流程同进程    | `flow_runtime` 节点 + LangGraph 编译执行 `graph_json`，无 iframe     |
...
> 画布仅经 **LangGraph** 执行；`flow_runtime` 为节点 registry，无第三方 `langflow` 包依赖。
...
| 风险                     | 影响         | 对策                                             |
| ---------------------- | ---------- | ---------------------------------------------- |
| 画布节点类型扩展不足            | 编译失败         | 在 `nodes/registry` 注册并在前端调色板暴露              |
...
| 需求文档                     | 当前实现                                            |
| ------------------------ | ----------------------------------------------- |
| 可视化编排（需求称 Langflow）   | React Flow + `flow_runtime` 节点 + LangGraph 执行   |
...
**状态：设计稿，未开发。** 完整方案见 **[agent-enhancement-langchain-stack.md](./agent-enhancement-langchain-stack.md)**。

| 层级 | 技术 | 作用 |
|------|------|------|
| L4 | React Flow + `graph_json` | 可视化编排（编译为 LangGraph） |
| L3 | LangGraph | RAG 图 + 画布编译（并行/条件分支，见 [flow-langgraph-compiler.md](./flow-langgraph-compiler.md)） |
| L2 | LangChain | **已接入** `app/ai_stack/langchain`（见 [langchain-integration.md](./langchain-integration.md)） |
| L1 | DeepAgents | **主智能体可选绑定多个子智能体**；有绑定时由 DeepAgents 做任务规划与委派（`planner=deepagents`） |

**平台内协同（已落地）**：`agt_sub_agent_bindings` + DeepAgents；**不是** A2A 协议。见 [internal-agent-orchestration.md](./internal-agent-orchestration.md)。

**A2A 互联（外部协议，分阶段）**：`agent_type=a2a`，基于 Agent Card / `/.well-known/agent-card.json` 对接外部 Agent。见 [a2a-protocol-integration.md](./a2a-protocol-integration.md)。与内部子智能体绑定**互斥**。

**类型字段**：`agt_agents.agent_type`：`custom`（默认）| `a2a`。
...
- [agent-enhancement-langchain-stack.md](./agent-enhancement-langchain-stack.md) — LangChain / LangGraph / DeepAgents 智能体增强（规划）
- [internal-agent-orchestration.md](./internal-agent-orchestration.md) — 平台内多智能体协同（DeepAgents）
- [a2a-protocol-integration.md](./a2a-protocol-integration.md) — A2A 协议外部互联（含 **§2 外部登记与互联宿主** 产品说明）
- [custom-agent-a2a-refs.md](./custom-agent-a2a-refs.md) — 自定义智能体引用外部 A2A（规则 + 规划）
- [flow-runtime.md](./flow-runtime.md) — 流程运行时命名与实现
